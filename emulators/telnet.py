...
class TelnetServer:
	...
	def handle_client(self, client_socket, address):
		...
		if username == config.get_service_by_name("telnet")["username"] and password == config.get_service_by_name("telnet")["password"]:
			...
			logger.info(f'"type":["user","allowed"],"kind":"alert","category":["authentication","intrusion_detection"],"dataset":"faitour.honeypot","action":"handle_client","reason":"User logged in","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"')
		elif username == "anonymous" and config.get_service_by_name("telnet")["allow_anonymous"]:
			logger.info(f'"type":["user","allowed"],"kind":"alert","category":["authentication","intrusion_detection"],"dataset":"faitour.honeypot","action":"handle_client","reason":"User logged in","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"')
		else:
			logger.info(f'"type":["user","denied"],"kind":"alert","category":["authentication","intrusion_detection"],"dataset":"faitour.honeypot","action":"handle_client","reason":"User logged in","outcome":"denied"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"')
	...
	def stop(self):
		logger.info(f'"type":["end"],"kind":"event","category":["process"],"dataset":"faitour.application","action":"stop","reason":"Telnet server emulator is stopping","outcome":"success"')
		self.running = False
		...
		self.clients = []
		logger.info(f'"type":["end"],"kind":"event","category":["process"],"dataset":"faitour.application","action":"stop","reason":"Telnet server emulator has stopped","outcome":"success"')
