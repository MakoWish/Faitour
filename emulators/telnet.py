...
class TelnetServer:
	...
	def start(self):
		if not os.path.exists(self.root_dir):
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"provider":"application","action":"handle_packet","reason":"Root directory {self.root_dir} does not exist","outcome":"failure"}},"error":{{"message":"Directory does not exist"')
			return
		...
		self.running = True
		logger.info(f'"type":["info"],"kind":"event","category":["process"],"provider":"application","action":"handle_packet","reason":"Telnet server started on {self.host_ip}:{self.host_port}","outcome":"success"')
		...
		while self.running:
			try:
				...
				logger.info(f'"type":["connection","allowed","start"],"kind":"alert","category":["network","intrusion_detection"],"provider":"honeypot","action":"start","reason":"New connection from","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}')
				client_thread = threading.Thread(target=self.handle_client, args=(client_socket, address))
				client_thread.start()
				self.clients.append((client_socket, client_thread))
			except Exception as e:
				logger.error(f'"type":["error"],"kind":"event","category":["process"],"provider":"application","action":"handle_packet","reason":"Server error","outcome":"failure"}},"error":{{"message":"{e}"')
	...
	def handle_client(self, client_socket, address):
		...
		if username == config.get_service_by_name("telnet")["username"] and password == config.get_service_by_name("telnet")["password"]:
			...
			logger.info(f'"type":["user","connection","allowed","start"],"kind":"alert","category":["authentication","network","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User logged in","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"')
			...
			while True:
				...
				if data.startswith("cd "):
					...
					if os.path.commonpath([self.root_dir, new_dir]) == self.root_dir and os.path.exists(new_dir) and os.path.isdir(new_dir):
						current_dir = new_dir
						logger.info(f'"type":["user","access","allowed"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User access to {new_dir} granted","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"file":{{"directory":"{new_dir}"')
					else:
						client_socket.send(b"Directory not found or access denied.\n")
						logger.error(f'"type":["user","access","denied"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User access to {new_dir} denied","outcome":"failure"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"file":{{"directory":"{new_dir}"')
				elif data == "ls":
					try:
						files = os.listdir(current_dir)
						response = "\n".join(files) + "\n"
						client_socket.send(response.encode())
						logger.info(f'"type":["user","access","allowed"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User listed directory {current_dir}","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"file":{{"directory":"{current_dir}"')
					except Exception as e:
						client_socket.send(f"Error listing directory: {e}\n".encode())
						logger.error(f'"type":["user","access","denied"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User attempt to list directory {current_dir} failed","outcome":"failure"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"error":{{"message":"{e}"}},"file":{{"directory":"{current_dir}"')
					...
					if os.path.commonpath([self.root_dir, filepath]) == self.root_dir and os.path.exists(filepath) and os.path.isfile(filepath):
						with open(filepath, 'r') as f:
							...
							logger.info(f'"type":["user","access","allowed"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User cat\'ed contents of file {filepath}","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"file":{{"name":"{filename}","directory":"{filepath}"')
					else:
						client_socket.send(b"File not found or access denied.\n")
						logger.error(f'"type":["user","access","denied"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User attempt to cat file {filepath} failed","outcome":"failure"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"error":{{"message":"File not found or access denied"')
				else:
					try:
						result = subprocess.check_output(data, shell=True, cwd=current_dir, stderr=subprocess.STDOUT)
						client_socket.send(result)
						logger.info(f'"type":["user","access","allowed"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User ran unhandled command {data}","outcome":"success"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"')
					except subprocess.CalledProcessError as e:
						client_socket.send(e.output or b"Command failed.\n")
						logger.error(f'"type":["user","access","denied"],"kind":"alert","category":["file","intrusion_detection"],"provider":"honeypot","action":"handle_client","reason":"User command {data} failed","outcome":"failure"}},"source":{{"ip":"{client_ip}","port":{client_port}}},"destination":{{"ip":"{self.host_ip}","port":{self.host_port}}},"user":{{"name":"{username}","password":"{password}"}},"error":{{"message":"{e}"')
		except Exception as e:
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"provider":"application","action":"handle_client","reason":"Client error","outcome":"failure"}},"error":{{"message":"{e}"')
		finally:
			client_socket.close()

	# Stops the telnet server.
	def stop(self):
		logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop","reason":"Telnet server emulator is stopping","outcome":"success"')
		self.running = False
		...
		self.clients = []
		logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop","reason":"Telnet server emulator has stopped","outcome":"success"')
