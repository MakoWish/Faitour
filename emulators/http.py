...
class LoginPageHandler(http.server.BaseHTTPRequestHandler):
	...
	def do_POST(self):
		...
		try:
			...
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"provider":"application","action":"do_POST","reason":"HTTP POST error","outcome":"failure"}},"error":{{"message":"{e}"')
		...
		if username == config.get_service_by_name("http")["username"] and password == config.get_service_by_name("http")["password"]:
			...
			logger.error(f'"type":["user","authentication","denied"],"kind":"alert","category":["network","intrusion_detection"],"provider":"honeypot","action":"login_fail","reason":"User login failed","outcome":"failure"}},"source":{{"ip":"{self.client_address[0]}","port":{self.client_address[1]}}},"destination":{{"ip":"{self.server.server_address[0]}","port":{self.server.server_address[1]}}},"http":{{"request":{{"method":"POST"}},"response":{{"status_code":401}}}},"url":{{"full":"{self.get_full_url()}","path":"{self.path}"}},"user":{{"name":"{username}","password":"{password}"')
			self.send_error_page(401)
...
class WebServer:
	...
	def start_http(self):
		if self.http_enabled:
			self.running = True
			logger.info(f'"type":["start"],"kind":"event","category":["process"],"provider":"application","action":"start_http","reason":"HTTP server emulator is starting on http://{self.host_ip}:{self.host_port}","outcome":"unknown"')
			TCPServer.allow_reuse_address = True
			httpd_http = TCPServer((self.host_ip, self.host_port), LoginPageHandler)
			httpd_http.serve_forever()
			logger.info(f'"type":["start"],"kind":"event","category":["process"],"provider":"application","action":"start_http","reason":"HTTP server emulator has started on http://{self.host_ip}:{self.host_port}","outcome":"success"')
	...
	def start_https(self):
		if self.https_enabled:
			...
			logger.info(f'"type":["start"],"kind":"event","category":["process"],"provider":"application","action":"start_http","reason":"HTTPS server emulator is starting on https://{self.host_ip}:{self.https_port}","outcome":"unknown"')
			TCPServer.allow_reuse_address = True
			httpd_https = TCPServer((self.host_ip, self.https_port), LoginPageHandler)
			logger.info(f'"type":["start"],"kind":"event","category":["process"],"provider":"application","action":"start_http","reason":"HTTPS server emulator has started on https://{self.host_ip}:{self.https_port}","outcome":"success"')
	...
	def stop_servers(self):
		...
		if self.httpd_http:
			logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop_servers","reason":"HTTP server emulator is stopping","outcome":"success"')
			self.httpd_http.shutdown()
			self.httpd_http.server_close()
			self.httpd_http = None
			logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop_servers","reason":"HTTP server emulator has stopped","outcome":"success"')

		# Stop the HTTPS server if it's running
		if self.httpd_https:
			logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop_servers","reason":"HTTPS server emulator is stopping","outcome":"success"')
			self.httpd_https.shutdown()
			self.httpd_https.server_close()
			self.httpd_https = None
			logger.info(f'"type":["end"],"kind":"event","category":["process"],"provider":"application","action":"stop_servers","reason":"HTTPS server emulator has stopped","outcome":"success"')
	...
	def generate_self_signed_cert(self):
		...
		if os.path.exists(cert_path) and os.path.exists(key_path):
			logger.debug(f'"type":["info"],"kind":"event","category":["configuration"],"provider":"application","action":"generate_self_signed_cert","reason":"HTTP certificate and key already exist","outcome":"success"')
			return
		...
		logger.debug(f'"type":["info"],"kind":"event","category":["configuration"],"provider":"application","action":"generate_self_signed_cert","reason":"HTTPS Self-signed certificate and key generated","outcome":"success"')
