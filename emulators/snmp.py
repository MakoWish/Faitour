...
class SNMPServer:
	...
	def configure(self):
		try:
			logger.info(f'"type":["start"],"kind":"event","category":["process"],"dataset":"application","action":"configure","reason":"SNMP server emulator is starting on {self.host_ip}:{self.host_port}","outcome":"success"')
			...
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"dataset":"application","action":"configure","reason":"Error configuring SNMP server","outcome":"failure"}},"error":{{"message":"{e}"')
			raise
	...
	def start(self):
		try:
			...
			if dispatcher is None:
				logger.error(f'"type":["error"],"kind":"event","category":["process"],"dataset":"application","action":"configure","reason":"Transport dispatcher not initialized","outcome":"failure"}},"error":{{"message":"Transport dispatcher not initialized"')

			# Job Started signals that dispatcher will process requests
			dispatcher.jobStarted(1)
			dispatcher.runDispatcher()
		except Exception as e:
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"dataset":"application","action":"start","reason":"SNMP server emulator error","outcome":"failure"}},"error":{{"message":"{e}"')
	
	# Stop the SNMP server gracefully.
	def stop(self):
		try:
			logger.info(f'"type":["end"],"kind":"event","category":["process"],"dataset":"application","action":"stop","reason":"SNMP server emulator is stopping","outcome":"success"')
			self.running = False
			dispatcher = self.snmp_engine.transportDispatcher

			if dispatcher:
				dispatcher.closeDispatcher()
				logger.info(f'"type":["end"],"kind":"event","category":["process"],"dataset":"application","action":"stop","reason":"SNMP server emulator has stopped","outcome":"success"')
		except Exception as e:
			logger.error(f'"type":["error"],"kind":"event","category":["process"],"dataset":"application","action":"stop","reason":"SNMP server emulator failed to stop","outcome":"failure"}},"error":{{"message":"{e}"')
